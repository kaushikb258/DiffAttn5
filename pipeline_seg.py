...
class SEGCFGSelfAttnProcessor:
    ...
    def __init__(self, blur_sigma=1.0, do_cfg=True, inf_blur_threshold=9999.0, attn_guid_option=1):
        ...
        if self.blur_sigma > inf_blur_threshold:
            ...
            self.inf_blur = False
        self.attn_guid_option = attn_guid_option    
        if self.attn_guid_option == 3:
            self.bilat = BilateralFilter(1, 5, 576, 576).cuda()
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # KAUSHIK ADDED THE FOLLOWING TWO LINES 
        # hidden_states = torch.Size([2, 576, 1280])
        # encoder_hidden_states = None    
        ...
        if self.do_cfg:
            ...
            hidden_states = scaled_dot_product_attention_kb(
            ...
            if self.attn_guid_option == 3:
                bilat = self.bilat
            hidden_states = apply_attention_kb(batch_size, input_ndim,  
                  hidden_states, encoder_hidden_states, attn, attention_mask, self.blur_sigma, self.inf_blur,
                   self.attn_guid_option, bilat)            
...
class StableDiffusionXLSEGPipeline(
    ...
    def __call__(
        ...
        attn_guid_option=0, # KAUSHIK ADDED THIS
        ...
        **kwargs,
        ...
        Args:
            ...
            attn_guid_option ('int', *optional*, defaults to 0): option to set attention guidance     
        ...
        if self.do_seg:
            ...
            #print(f"number of down layers: {len(down_layers)}")
            #print(f"number of mid layers: {len(mid_layers)}")
            #print(f"number of up layers: {len(up_layers)}")       
            #number of down layers: 24
            #number of mid layers: 10
            #number of up layers: 36     
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.do_seg:
                    ...
                    replace_processor = SEGCFGSelfAttnProcessor(blur_sigma=seg_blur_sigma, 
                        do_cfg=self.do_classifier_free_guidance, attn_guid_option=attn_guid_option)
